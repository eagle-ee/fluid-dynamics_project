...
void CChildView::OnPaint() 
{
	...
	row += 20;
	MemDC1.TextOutW(8, row, _T("J : Subtract viscosity"));
	row += 20;
	MemDC1.TextOutW(8, row, _T("K : Add viscosity"));
...
void CChildView::OnKeyDown(UINT nChar, UINT nRepCnt, UINT nFlags)
{
	...
	case 'j':
	case 'J':
		fluidSolver.setViscosity(fluidSolver.getViscosity() * .8);
		Invalidate(false);
	case 'k':
	case 'K':
		fluidSolver.setViscosity(fluidSolver.getViscosity() * 1.2);
		Invalidate(false);
	}
